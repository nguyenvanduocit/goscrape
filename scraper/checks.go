...
func (s *Scraper) shouldURLBeDownloaded(url *url.URL, currentDepth uint, isAsset bool) bool {
	...
	if url.Host != s.URL.Host && s.config.SkipExternalResources {
		s.logger.Info("Skipping external host page", log.String("url.Host", url.String()), log.String("s.URL.Host", s.URL.Host))
		return false
	}
