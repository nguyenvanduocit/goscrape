...
func (s *Scraper) fixNodeURL(baseURL *url.URL, attributeName string, node *html.Node,
	...
	if !strings.Contains(nodeURL, s.URL.Host) && s.config.SkipExternalResources {
		s.logger.Info("Skipping external resource", log.String("url", nodeURL))
		return false
	}

	// trim trailing slash and index.html
	resolved = strings.TrimRight(resolved, PageDirIndex)

	s.logger.Info("HTML Element relinked",
